...
const getWidgetUpdate = async () => {
  ...
  if (!result) {
    let sql = "select id, log_type, title, version, post_date, content_type, description from changelog order by post_date desc limit 3";
...
const getSiteUpdate = async (pageInfo) => {
  let sql = "select id, version, post_date, content_type, title, description as highlight, details as description from changelog where log_type = 1 order by post_date desc limit ?, ?";
